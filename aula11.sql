...
INSERT INTO
    tbl_departamentos (nome, data_inicio, cod_cidade)
VALUES
...
INSERT INTO
    tbl_projetos (nome, cod_departamento)
VALUES
    ('Projeto A', 1),
    ('Projeto B', 2),
    ('Projeto C', 2),
...
CREATE PROCEDURE proc_upd_nome_depart(cod_departamento INT, novonome TEXT) LANGUAGE PLPGSQL AS $$ BEGIN
UPDATE
    tbl_departamentos
SET
    nome = novonome
...
CREATE
OR REPLACE PROCEDURE proc_copiatbl() LANGUAGE PLPGSQL AS $$ BEGIN CREATE TABLE IF NOT EXISTS tbl_cidades_copia AS
SELECT
    *
FROM
    tbl_cidades;
...
CREATE
OR REPLACE PROCEDURE proc_novoprojeto(nome_projeto TEXT, cod_departamento INT) LANGUAGE PLPGSQL AS $$ BEGIN
INSERT INTO
    tbl_projetos (nome, cod_departamento)
VALUES
    (nome_projeto, cod_departamento);
...
CREATE
OR REPLACE PROCEDURE proc_delprojeto(cod_projeto INT) LANGUAGE PLPGSQL AS $$ BEGIN
DELETE FROM
    tbl_projetos
WHERE
...
-- exercicios
CREATE
OR REPLACE PROCEDURE proc_projeto_arquivado(cod_projeto INT) LANGUAGE PLPGSQL AS $$ BEGIN CREATE TABLE IF NOT EXISTS tbl_projetos_arquivados (
    codigo_projeto SERIAL PRIMARY KEY,
    nome TEXT
);

INSERT INTO
    tbl_projetos_arquivados (nome)
SELECT
    nome
FROM
    tbl_projetos
